...
async function fetchAndCacheFootballRows(dateStr: string): Promise<Array<{ source_match_key: string; league_name: string }>> {
  const sourceDates = [dateStr]; // Only use the current date page — previous day pages are titan007's own convention and should not leak into next day schedule
  ...
  // Cache results: 30 min for non-empty, 5 min for empty (far-future dates)
  const cacheTtl = rows.length > 0 ? 30 * 60 * 1000 : 5 * 60 * 1000;
  footballImportantRowsCache.set(dateStr, { expiresAt: Date.now() + cacheTtl, rows });
  return rows;
}
...
async function getFootballImportantRows(dateStr: string) {
  ...
  if (cached) {
    ...
    if (cached.rows.length > 0) prewarmAdjacent(dateStr);
    return cached.rows;
  }
...
async function getRemoteTitanBasketballTeams(category: string, leagueId: number) {
  ...
  // No cache at all: seed cache with DB teams immediately (fast), then refresh from titan007 in background
  const dbTeams = getDbTeamsByCategory(category);
  remoteTeamPoolCache.set(cacheKey, { expiresAt: Date.now() + 5 * 60 * 1000, teams: dbTeams });
  if (!remoteTeamPoolRefreshing.has(cacheKey)) {
    ...
    void (async () => {
      try {
        ...
        // titan007 unreachable — DB teams remain cached, will retry after 5 min TTL
      } finally {
        remoteTeamPoolRefreshing.delete(cacheKey);
      }
    })();
...
// Pre-warm caches on module load so first user request is always fast
void (async () => {
  try {
    // Seed NBA/韩篮甲 team caches from DB immediately (no external call)
    const titanLeagueMap: Record<string, number> = { NBA: 1, "韩篮甲": 15 };
    for (const cat of Object.keys(titanLeagueMap)) {
      const season = buildBasketballSeason();
      const key = `${cat}:${season}`;
      if (!remoteTeamPoolCache.has(key)) {
        const teams = getDbTeamsByCategory(cat);
        if (teams.length > 0) {
          remoteTeamPoolCache.set(key, { expiresAt: Date.now() + 5 * 60 * 1000, teams });
        }
      }
    }
    // Pre-warm football important rows for today in background
    const now = new Date();
    ...
    getFootballImportantRows(todayStr).catch(() => {});
  } catch {}
})();
